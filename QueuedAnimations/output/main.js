...
var $author$project$Main$initialStyle = $mdgriffith$elm_style_animation$Animation$style(
	_List_fromArray(
		[
			$mdgriffith$elm_style_animation$Animation$scale(0.3),
			$mdgriffith$elm_style_animation$Animation$left(
			$mdgriffith$elm_style_animation$Animation$px(410.0)),
			$mdgriffith$elm_style_animation$Animation$borderRadius(
			$mdgriffith$elm_style_animation$Animation$px(360.0)),
			$mdgriffith$elm_style_animation$Animation$opacity(0)
		]));
var $author$project$Main$initialModel = {searchPhrase: '', style: $author$project$Main$initialStyle, url: ''};
...
var $author$project$Main$GoingRight = {$: 'GoingRight'};
var $author$project$Main$defaultPhoto = {id: '1', title: '', url: ''};
...
var $author$project$Main$RunHTTPChain = function (a) {
	return {$: 'RunHTTPChain', a: a};
};
...
var $author$project$Main$getPicturesByUID = function (userId) {
	return $elm$http$Http$task(
		{
			...
			url: $author$project$Main$buildUrl(userId)
		});
};
...
var $author$project$Main$getUserAndPhotos = function (username) {
	return A2(
		$elm$core$Task$attempt,
		$author$project$Main$RunHTTPChain,
		A2(
			$elm$core$Task$andThen,
			function (_v0) {
				return $author$project$Main$getPicturesByUID('abc');
			},
			$author$project$Main$getUserId(username)));
};
...
var $author$project$Main$update = F2(
	function (message, model) {
		switch (message.$) {
			case 'SetSearchPhrase':
				var phrase = message.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{searchPhrase: phrase}),
				...
				var newStyle = A2(
					...
					_List_fromArray(
						[
							...
							$mdgriffith$elm_style_animation$Animation$Messenger$send($author$project$Main$GoingRight),
							$mdgriffith$elm_style_animation$Animation$to(
							_List_fromArray(
								[
									$mdgriffith$elm_style_animation$Animation$left(
									$mdgriffith$elm_style_animation$Animation$px(140.0))
								]))
						]),
					model.style);
				return _Utils_Tuple2(
			...
			case 'Search':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{style: $author$project$Main$initialStyle, url: ''}),
					$author$project$Main$getUserAndPhotos(model.searchPhrase));
			case 'RunHTTPChain':
				if (message.a.$ === 'Err') {
					var err = message.a.a;
					return _Utils_Tuple2(
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								url: A2($elm$core$Maybe$withDefault, $author$project$Main$defaultPhoto, head).url
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'Animate':
...
var $author$project$Main$SetSearchPhrase = function (a) {
	return {$: 'SetSearchPhrase', a: a};
};
...
var $author$project$Main$onLoad = function (message) {
	return A2(
		$elm$html$Html$Events$on,
		'load',
...
var $author$project$Main$renderImg = F2(
	function (url, style) {
		return $elm$core$String$isEmpty(url) ? A2(
			$elm$html$Html$p,
			_List_Nil,
			_List_fromArray(
				[
					$elm$html$Html$text('no photo for empty url')
				])) : A2(
			$elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
					...
					$elm$core$List$concat(
						_List_fromArray(
							[
								...
								_List_fromArray(
								[
									$author$project$Main$onLoad($author$project$Main$ImageLoaded)
								])
							])),
					_List_Nil)
				]));
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Events$onInput($author$project$Main$SetSearchPhrase)
					]),
				_List_Nil),
				A2(
				...
				A2($author$project$Main$renderImg, model.url, model.style)
			]));
};
